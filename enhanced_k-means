...
# Enhanced k-means with additional traits
...
# Enhanced k-means loop with custom convergence criteria
def enhanced_kmeans(data, k, initial_centroids, max_iter, tolerance=1e-4, record_heterogeneity=None, verbose=False):
    centroids = initial_centroids[:]
    prev_cluster_assignment = None
    ...
    for itr in range(max_iter):  
        ...
        if prev_heterogeneity - current_heterogeneity < tolerance:
            if verbose:
                print(f'Convergence reached at iteration {itr}')
            break
...
# Run enhanced k-means with k=3 clusters for a maximum of 400 iterations and plot the heterogeneity
k = 3
heterogeneity = []
initial_centroids = k_means_plus_plus_initialization(tf_idf, k, seed=0)
centroids, cluster_assignment = enhanced_kmeans(tf_idf, k, initial_centroids, max_iter=400, tolerance=1e-4, record_heterogeneity=heterogeneity, verbose=True)
...
def kmeans_multiple_runs(data, k, max_iter, seeds, tolerance=1e-4, verbose=False):
    ...
    for seed in seeds:
        ...
        centroids, cluster_assignment = enhanced_kmeans(data, k, initial_centroids, max_iter=max_iter, tolerance=tolerance, verbose=verbose)
...
for k in k_list:
    print(f'Running k = {k}')
    all_centroids[k], all_cluster_assignment[k] = kmeans_multiple_runs(tf_id

f, k, max_iter=400, seeds=seeds, tolerance=1e-4, verbose=True)
...
In this enhanced version, I added the `enhanced_kmeans` function, which includes k-means++ initialization and a custom convergence criterion based on changes in heterogeneity. This should improve the chances of successful clustering. The project also includes visualization and multiple runs to ensure robust clustering results.
